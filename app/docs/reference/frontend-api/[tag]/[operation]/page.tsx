...
export default async function Page(props: PageProps<'/docs/reference/frontend-api/[tag]/[operation]'>) {
...
export async function generateMetadata(props: PageProps<'/docs/reference/frontend-api/[tag]/[operation]'>): Promise<Metadata> {
  const { tag, operation } = await props.params;
  const tagEntry = frontendManifest.tags.find(t => t.slug === tag);
  const opEntry = tagEntry?.operations.find(op => op.slug === operation);
  if (!opEntry) {
