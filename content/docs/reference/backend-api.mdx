...
### Authentication — 41 operations
`API Auth` · `Session` · `OAuth` · `JWT Templates` · `Signing Keys` · `Session Tickets`

Create and manage API keys with configurable scopes. List and revoke active sessions across users. Administer OAuth applications and their clients — create apps, manage secrets, and configure redirect URIs. Rotate OIDC signing keys per OAuth app, or mark them compromised on suspected leak. Configure JWT template overrides for custom claims. Issue server-side session tickets (impersonation, agent access, webhook-app access, API-auth access) that hand the user a short-lived redeemable URL.
