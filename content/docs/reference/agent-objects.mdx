...
This reference is for engineers wiring the agent runtime through the platform API, `@wacht/backend`, or the bench CLI. If you're using the console UI, see the [Agents guide](/guides/agents) instead.
...
| Actor project | `id` (per deployment); binds one actor + one agent | `createActorProject` (actor id + `{ agent_id, name }`) |
...
| Session ticket | one-time `ticket` string | `createSessionTicket` |
...
- `interval` — runs at `next_run_at`, then repeats every `interval_seconds` (minimum 600).
...
`wacht api describe createAiTool` shows the full per-variant payload schema for the persisted variants. See [Composio](/guides/agents/composio) for the Virtual flow.
...
`POST /session/tickets` (`createSessionTicket`) issues four ticket types — each redirects to a different vanity surface:
...
Tickets are one-time-redeemable and bounded by `expires_in` (seconds). See the [Agent sessions guide](/guides/agents/sessions) for scoping fields and the per-product vanity-pages guides ([API auth](/guides/api-auth/vanity-pages-implementation), [Webhooks](/guides/webhook-apps/vanity-pages-implementation)) for non-agent flows.

## Related

- [Agents](/guides/agents) — user-facing overview.
- [Scheduling](/guides/agents/scheduling)
- [Composio](/guides/agents/composio)
- [Agent sessions](/guides/agents/sessions)
