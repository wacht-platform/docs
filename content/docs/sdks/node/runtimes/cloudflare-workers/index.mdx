...
Leave the API base URL unset — the SDK defaults to `https://api.wacht.dev`. Only set `WACHT_BACKEND_API_URL` if Wacht gives you a different base; never point it at a deployment frontend host.
...
See [JWT and API Key/OAuth Protected Auth](/sdks/node/runtimes/cloudflare-workers/auth-and-gateway) for full implementations.
