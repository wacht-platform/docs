...
## `uiOverrides` lets you override deployment UI settings

The Next.js `DeploymentProvider` accepts a `uiOverrides` prop:
...
If you are embedding things like sign-in, sign-up, account, or profile flows directly in your application, `uiOverrides` gives you a way to adjust the UI-facing deployment settings for that embedded experience without changing the deployment itself.
...
Use `uiOverrides` when you want to adjust the UI behavior for a specific app shell without changing the deployment itself.
