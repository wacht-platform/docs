---
title: Server Auth
description: Authenticate React Router requests, forward the returned headers, and use the privileged server client when you need backend access.
---

The React Router server entrypoint is `@wacht/react-router/server`.
...
That is the helper to reach for in loaders and actions when you need to keep server and client state aligned.
...
When middleware or a server helper has already serialized the auth state into headers, `authFromHeaders()` turns that back into the normalized auth object.

That keeps request auth consistent across loader, action, and rendering code.
...
```ts
import { wachtClient } from '@wacht/react-router/server';
