...
You need an OAuth app and at least one OAuth client. If you've already done the [OAuth Apps create flow](/guides/oauth-apps/create-oauth-app-and-clients), skip ahead.
...
Access token format is a per-client setting. See [Verify OAuth Access Tokens](/guides/oauth-apps/access-token-verification) for how to verify either format and which one to pick.
...
If the access_token is opaque (the default), verify it with `/oauth/userinfo` or `/oauth/introspect`. If you've set the client to JWT access tokens, verify locally against the JWKS — see [Verify OAuth Access Tokens](/guides/oauth-apps/access-token-verification).
