...
description: "Vendor-agnostic OIDC for relying parties — how to authenticate users through Wacht using any standard OIDC client library."
---

# Use Wacht as an OpenID Connect Provider

If you want to sign users into your application through Wacht without committing to the Wacht SDK, you can talk to Wacht over plain OpenID Connect.

Every OAuth app you create in Wacht is an OIDC provider out of the box. There's no separate toggle, no second product to enable, no deployment-level flag. The same OAuth FQDN you'd use for OAuth 2.x also serves `.well-known/openid-configuration`, `.well-known/jwks.json`, `/oauth/userinfo`, and `/oauth/logout`.

This guide is for relying parties — your app, your backend, your OIDC client library. If you're building Wacht's first-party UI you probably want the SDKs instead.

## What you get without doing anything special

1. **Auth code flow with PKCE** — the only flow Wacht implements, and the only one most RPs need.
2. **Discovery + JWKS** — your library can self-configure from `https://<oauth-fqdn>/.well-known/openid-configuration`.
...
4. **Per-OAuth-app RSA signing keys** — provisioned at app creation time, rotatable from console without code changes.
...
7. **Refresh tokens with rotation + replay detection** — captured-token reuse triggers full family revocation.
8. **`prompt` and `max_age` parameter handling** — `none`, `login`, `consent`, `select_account` are all wired through; `max_age` is enforced against the actual authentication time.

## What's intentionally **not** implemented

These don't change what a normal RP can do, but it's good to know upfront:

1. **Implicit and hybrid flows** — code flow only. RFC 9700 recommends this anyway.
2. **Pairwise subject identifiers** — every RP sees the same `sub` for the same user.
3. **`claims` request parameter / request objects / `request_uri`** — out of scope for a code-flow OP.
4. **Front-channel / back-channel logout** — only RP-Initiated logout is wired.
5. **Signed or encrypted UserInfo responses** — JSON over HTTPS only.

If your client library expects any of these, it should still work — these features are optional in OIDC Core and most libraries don't require them.
...
6. **Wacht returns** an opaque `access_token`, an opaque `refresh_token` (if `offline_access` was requested or the client has `refresh_token` grant enabled), and a signed `id_token`.
...
If your access_token is opaque too — and it is — verify it with `/oauth/userinfo` or `/oauth/introspect`, not by treating it as a JWT.
...
A 200-line zero-dep Node script that walks the entire flow lives at `platform-api/oidc-test.mjs` in the Wacht repo. It opens a browser, listens on localhost, exchanges the code, verifies the id_token against JWKS, calls UserInfo, refreshes, replays the old refresh token to confirm replay detection fires, and exercises logout. Use it as a reference when integrating your own library.

```bash
# Edit OAUTH_HOST + CLIENT_ID at the top, then:
node oidc-test.mjs

# Try variants:
node oidc-test.mjs prompt=consent
node oidc-test.mjs prompt=none
node oidc-test.mjs max_age=10
```
