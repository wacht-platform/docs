...
2. **MCP toggle** (`require_approval_mcp`). When `true`, every tool whose name starts with `mcp_` resolves to `review`.
3. **Virtual toggle** (`require_approval_virtual`). When `true`, every tool whose name starts with `v_` resolves to `review`.
4. **Per-attachment action** on `ai_agent_tools.approval_action`. Only applies to tools the operator has explicitly attached to the agent (created via `POST /ai/tools` and joined via `/agents/{id}/tools/{tool_id}`).
5. Default: `allow`.
...
Namespace toggles for MCP and virtual tools also take priority over the per-tool dropdown.
