...
description: Route work across specialist agents with a coordinator that decides which lane runs next.
---
...
- **Tools:** none required, but `get_project_task` (returns the board item with its assignments and deliverables) and `list_threads` are useful for reading state
- **System prompt:**
...
const project = await ai.createActorProject(userActorId, {
  agent_id: "video_coordinator_agent_id",
  name: "Video for X",
});
...
3. **Script lane runs.** Writes script to `/task/artifacts/script.txt`, finishes its slice with a handoff (summary, artifact path, next: "storyboard") that routes the board item back to the coordinator.
4. **Coordinator wakes up again.** Sees the finished assignment, reads the journal entry, decides: "script done, storyboard next."
...
- **Board state** — `get_project_task` returns the board item's assignments, so the coordinator sees which lanes have run, which are active, which are pending.
...
## Fan-out across a lane

If a lane needs many parallel pieces (image-gen producing 8 key frames), route one task per piece to the same lane thread. `delegateProjectTask` takes four fields — `target_lane_thread_id`, `title`, `description`, `capability_tags` — and creates a board item assigned to a matching agent on that lane.

<Tabs items={["Node SDK", "Rust SDK"]}>
<Tab>
```ts
// Route one task per shot at the image-gen lane thread
...
Each delegated task is its own board item; they run in parallel, subject to the lane's concurrency limits. To make the originating task wait on the batch, the agent creates them as child tasks in-loop with the `create_project_task` tool (passing `parent_task_key`) rather than from the SDK — the parent then sits in `waiting_for_children` until they all finish.
