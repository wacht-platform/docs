...
- [Model overrides](/guides/agents/model-overrides) — pick a different strong or weak model for one agent.
- [Execution hooks](/guides/agents/hooks) — run setup or cleanup tools around every run.
- [Approval policy](/guides/agents/approval-policy) — gate risky tool calls behind review.
- [Scheduling](/guides/agents/scheduling) — recurring or one-shot agent work.
- [Composio integrations](/guides/agents/composio) — wire Reddit, Gmail, Slack, and other third-party tools.
- [Agent sessions](/guides/agents/sessions) — share a chat URL.
...
This separation lets one agent be reused across many tasks and schedules. If you're calling the platform API directly (`@wacht/backend`, bench CLI), each layer is its own object — see the [agent runtime reference](/reference/agent-objects).
