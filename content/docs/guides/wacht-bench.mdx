...
Each call issues a new key under a deployment-scoped system app. Existing keys keep working — re-pulling on a teammate's machine doesn't invalidate yours. Pass `--file <path>` to override the destination or `--print` to dump credentials to stdout without writing.
