...
```bash
wacht skills install
```

Install one skill:

```bash
wacht skills install --skill wacht-nextjs-patterns
```

Install into specific agents (skips the interactive picker):

```bash
wacht skills install --agent claude-code,cursor,codex
wacht skills install --all-agents
```

Install at user scope instead of project scope with `--global`. Copy files instead of symlinking with `--copy`.
